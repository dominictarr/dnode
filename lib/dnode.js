...
/*
  TODO:process verbosity
    - 0 silent
    - 1 localError
    - 2 remoteError
    - 3 all errors
    - 4 all events (connect,ready,end)

  add logger as listener for events.
  handle all in seperate file, for tidyness.
...
function DNode (wrapper) {
    ...
    self.connect = function () {
        args = pa(arguments)
        var stream = args.server;

        if (args.port) {
            stream = net.createConnection(args.port, args.host);
            stream.remoteAddress = args.host || '127.0.0.1';
            stream.remotePort = args.port;
        }
        
        self.withStream(stream, args, args.block);
    ...
    self.withStream = function (stream, opts, block) {
        ...
        conn.on('remote', function (remote) {
            if (block) {
                ...
                if (opts.ping) pingInterval(conn, 100, 50);//i've just hardcoded a sensible default.
            }
        });
    ...
    self.listen = function () {
        var args = pa(arguments)
        var server = args.server
        
        if (server instanceof io.Listener) {
            var stream = StreamSocketIO(function (stream) {
                self.withStream(stream, kwargs, args.block);//when is the block invoked? on ready, on connect?
            });
        }
        //there is no test for using http.Server
        else if (server instanceof http.Server) {
            // http server to proxy socketIO connections with
            delete args.server;
            var sock = io.listen(server, args);
        ...
        self.running = function (){
          console.log(sys.inspect(server,true))
